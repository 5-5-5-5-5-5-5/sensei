  ...
  aplicar: (src: string, relPath: string, ast: NodePath<Node> | null): Ocorrencia[] => {
    ...
    try {
      ...
      for (const [prioridade, items] of Object.entries(porPrioridade)) {
        if (items.length > 0) {
          ...
          ocorrencias.push(criarOcorrencia({
            ...
            nivel: nivelAjustado,
            mensagem: messages.DetectorAgregadosMensagens.problemasDocumentacaoResumo(prioridade, resumo, items.length),
            relPath,
            linha: items[0].linha
          }));
        }
      ...
      return [criarOcorrencia({
        tipo: 'ERRO_ANALISE',
        nivel: 'aviso',
        mensagem: messages.DetectorAgregadosMensagens.erroAnalisarDocumentacao(erro),
        relPath,
        linha: 1
      })];
    }

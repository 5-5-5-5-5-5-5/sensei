...
const isSvgCaminho = typeof fileCaminho === 'string' ? /\.svg$/i.test(fileCaminho) : undefined;
if (isSvgCaminho === false) return false; // Se tem caminho e não é .svg, ignora
const pareceSvg = /<svg\b/i.test(fullCode);
if (isSvgCaminho === undefined && !pareceSvg) return false;

...
export const LogMensagens = {
  sistema: {
    ...
    shutdown: `${ICONES_STATUS.ok} Analysis completed gracefully`,
    atualizacao: {
    ...
    poda: {
      ...
      concluida: `${ICONES_STATUS.ok} Pruning completed: Orphan files removed successfully!`
    },
    reversao: {
    ...
    auto: {
      mapa_reversao: {
        erro_carregar: `${ICONES_STATUS.falha} Error loading reversal map: {erro}`,
        erro_salvar: `${ICONES_STATUS.falha} Error saving reversal map: {erro}`,
        move_nao_encontrado: `${ICONES_STATUS.falha} Move not found: {id}`,
        arquivo_destino_nao_encontrado: `${ICONES_STATUS.falha} Destination file not found: {destino}`,
      ...
      poda: {
        nenhum_arquivo: `${ICONES_STATUS.ok} No files to prune this cycle.\n`,
  ...
  conselheiro: {
    volume_alto: `${ICONES_FEEDBACK.info} High volume of tasks? The code doesn't run away; burnout does.`,
    ...
    madrugada: `${ICONES_FEEDBACK.atencao} It's already past {hora}. Code compiles tomorrow; you rest now.`
  },
  guardian: {
  ...
  core: {
    parsing: {
      ...
      plugins_registrados: `${ICONES_DIAGNOSTICO.inicio} Default plugins registered in system`,

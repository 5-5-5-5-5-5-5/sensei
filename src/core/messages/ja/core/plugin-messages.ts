...
export const NextJsMensagens = {
  nextServerWithoutEnvCheck: 'Next.jsサーバー関数はNODE_ENVを確認する必要があります',
  nextMetadataIncomplete: 'Metadataにはtitleまたはdescriptionを含める必要があります'
} as const;
  /* -------------------------- Vueメッセージ -------------------------- */
export const VueMensagens = {
  vueVHtmlUnsafe: 'v-htmlはユーザー入力をバインドする場合不安全です',
  vueTwoWayBindingWithoutEmit: '双方向バインディングではプロパティの変更にemitを使用する必要があります'
} as const;
  /* -------------------------- Electronメッセージ -------------------------- */
export const ElectronMensagens = {
  electronNodeIntegration: 'nodeIntegration: trueはセキュリティリスクです',
  electronContextIsolation: 'contextIsolation: falseはpreloadをrendererに公開します',
  electronEvalUsage: 'Electronアプリでのeval()の使用はセキュリティリスクです',
  electronSyncIpc: '同期IPCはrendererプロセスをブロックする可能性があります'
} as const;
  /* -------------------------- REACT HOOKSメッセージ -------------------------- */

...
export const LogMensagens = {
  sistema: {
    ...
    performance: {
      regressao_detectada: `${ICONES_FEEDBACK.atencao} Regression above {limite}% 检测到.`,
    ...
    reversao: {
      nenhum_move: `${ICONES_STATUS.falha} No move found for 文件: {arquivo}`,
      revertendo: `${ICONES_COMANDO.reverter} Reverting moves for: {arquivo}`,
      sucesso: `${ICONES_STATUS.ok} File reverted 成功fully: {arquivo}`,
      falha: `${ICONES_STATUS.falha} Failed to revert 文件: {arquivo}`
    },
    auto: {
      mapa_reversao: {
        erro_carregar: `${ICONES_STATUS.falha} 错误 loading reversal map: {erro}`,
        erro_salvar: `${ICONES_STATUS.falha} 错误 saving reversal map: {erro}`,
        move_nao_encontrado: `${ICONES_STATUS.falha} Move not found: {id}`,
        arquivo_destino_nao_encontrado: `${ICONES_STATUS.falha} Destination 文件 not found: {destino}`,
        arquivo_existe_origem: `${ICONES_FEEDBACK.atencao} 文件 already exists at origin: {origem}`,
        erro_reverter: `${ICONES_STATUS.falha} 错误 reverting move: {erro}`,
      ...
      poda: {
        nenhum_arquivo: `${ICONES_STATUS.ok} No 文件 to prune this cycle.\n`,
        podando: `${ICONES_COMANDO.podar} Pruning {quantidade} 文件...`,
        podando_simulado: `${ICONES_COMANDO.podar} Pruning {quantidade} 文件... (SIMULATED)`,
        arquivo_movido: `${ICONES_STATUS.ok} {arquivo} 已移至 abandoned.`
      },
      corretor: {
    ...
    correcoes: {
      ...
      arquivo_nao_encontrado: `${ICONES_FEEDBACK.atencao} 文件 not found for correction: {arquivo}`,
  ...
  analistas: {
    execucao: {
      ...
      erro: `${ICONES_STATUS.falha} 错误 in analyst '{analista}': {erro}`,
      skip: `${ICONES_STATUS.pulado} Skipping '{arquivo}' (suppressed by configuration)`
    },
    metricas: {
      performance: `${ICONES_DIAGNOSTICO.stats} Performance: {analistas} analysts, {media}ms/文件 average`,
  ...
  filtros: {
    incluindo: ` ${ICONES_ACAO.criar} Including: {模式} ({matches} files)`,
    excluindo: ` ${ICONES_ACAO.deletar} Excluding: {模式} ({matches} files)`,
  ...
  projeto: {
    detectado: `${ICONES_RELATORIO.lista} Project 检测到: {tipo} ({confianca}% confidence)`,
    estrutura: `${ICONES_DIAGNOSTICO.stats} Structure: {arquivos} 文件, {linguagens} languages`,
  ...
  contexto: {
    desenvolvedor_novo: `${ICONES_FEEDBACK.info} Simple project 检测到 - simplified logs enabled`,
    equipe_experiente: `${ICONES_FEEDBACK.info} Enterprise project 检测到 - detailed logs enabled`,
    ci_cd: `${ICONES_FEEDBACK.info} CI/CD environment 检测到 - structured logs enabled`,
  ...
  conselheiro: {
    volume_alto: `${ICONES_FEEDBACK.info} High volume of tasks? The 代码 doesn't run away; burnout does.`,
    ...
    madrugada: `${ICONES_FEEDBACK.atencao} It's already past {hora}. 代码 compiles tomorrow; you rest now.`
  },
  guardian: {
    ...
    diferencas_detectadas: `${ICONES_RELATORIO.error} Differences 检测到:`,
    ...
    integridade_preservada: `${ICONES_STATUS.ok} No differences 检测到. Integrity preserved.`,
  ...
  auto: {
    plugin_ignorado: `${ICONES_NIVEL.aviso} 插件 ignored ({plugin}): {erro}`,
    caminho_nao_resolvido: `${ICONES_NIVEL.aviso} 插件 path not resolved: {plugin}`,
    plugin_falhou: `${ICONES_STATUS.falha} 插件 failed: {plugin} — {erro}`,
    move_removido: `${ICONES_ACAO.deletar} Move removed from map: {id}`
  },
  core: {
    parsing: {
      erro_babel: `${ICONES_NIVEL.aviso} Babel parsing 错误 in {arquivo}: {erro}`,
      erro_ts: `${ICONES_NIVEL.aviso} TS compiler parse 错误 in {arquivo}: {erro}`,
      erro_xml: `${ICONES_NIVEL.aviso} XML parse 错误 in {arquivo}: {erro}`,
      erro_html: `${ICONES_NIVEL.aviso} HTML parse 错误 in {arquivo}: {erro}`,
      erro_css: `${ICONES_NIVEL.aviso} CSS parse 错误 in {arquivo}: {erro}`,
      ...
      plugin_nao_encontrado: `${ICONES_NIVEL.aviso} 插件 not found for {extensao}, using legacy system`,
      sistema_plugins_falhou: `${ICONES_STATUS.falha} 插件 system failed: {erro}, using legacy system`,
      plugins_registrados: `${ICONES_DIAGNOSTICO.inicio} Default 插件s registered in system`,
      usando_plugin: `${ICONES_DIAGNOSTICO.inicio} Using 插件 '{nome}' for {extensao}`
    },
    plugins: {
      erro_carregar: `${ICONES_STATUS.falha} Error loading 插件 {nome}: {erro}`,
      ...
      extensao_nao_suportada: `${ICONES_NIVEL.aviso} Extension {extensao} not supported by core 插件`,
      registrando: `${ICONES_DIAGNOSTICO.inicio} Registering 插件: {nome} v{versao}`
    },
    executor: {

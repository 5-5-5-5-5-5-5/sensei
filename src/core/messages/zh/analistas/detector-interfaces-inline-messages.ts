...
export const DetectorInterfacesInlineMensagens = {
  ...
  interfaceExportadaParaTipos: (nomeInterface: string, tiposDir = 'src/tipos') => `导出ed interface '${nomeInterface}' should be in ${tiposDir.endsWith('/') ? tiposDir : `${tiposDir}/`}`,
  interfaceComplexaParaTipos: (nomeInterface: string, tiposDir = 'src/tipos') => `Complex interface '${nomeInterface}' should be 已移至 ${tiposDir.endsWith('/') ? tiposDir : `${tiposDir}/`}`,

...
export const NextJsMensagens = {
  nextServerWithoutEnvCheck: 'Next.js 服务器函数应检查 NODE_ENV',
  nextMetadataIncomplete: 'Metadata 应包含 title 或 description'
} as const;
  /* -------------------------- Vue 消息 -------------------------- */
export const VueMensagens = {
  vueVHtmlUnsafe: 'v-html 在绑定用户输入时可能不安全',
  vueTwoWayBindingWithoutEmit: '双向绑定应使用 emit 来处理属性变更'
} as const;
  /* -------------------------- Electron 消息 -------------------------- */
export const ElectronMensagens = {
  electronNodeIntegration: 'nodeIntegration: true 是安全风险',
  electronContextIsolation: 'contextIsolation: false 会将 preload 暴露给 renderer',
  electronEvalUsage: '在 Electron 应用中使用 eval() 是安全风险',
  electronSyncIpc: '同步 IPC 可能阻塞 renderer 进程'
} as const;
  /* -------------------------- React Hooks 消息 -------------------------- */

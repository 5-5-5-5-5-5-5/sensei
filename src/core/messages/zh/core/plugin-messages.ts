...
export const TailwindMensagens = {
  ...
  repeatedClass: (token: string) => `Repeated class 检测到 (${token}). Consider removing redundancy.`,
  importantUsage: (token: string) => `Use of ! (important) 检测到 in (${token}). Prefer utility classes or scope reinforcement instead of important.`,
...
export const CssMensagens = {
  duplicatePropertySame: (prop: string) => `Duplicated property with identical value (${prop}): error 检测到.`,
  duplicatePropertyDifferent: (prop: string, prev: string, curr: string) => `Duplicated property (${prop}) with different values. Possible 错误: "${prev}" vs "${curr}".`,
  ...
  idSelectorPreferClass: (selector: string) => `ID selector (${selector}) 检测到. For reuse and consistency, prefer classes when possible.`,
  ...
  cssHackDetected: (hack: string) => `CSS hack 检测到 (${hack}). Consider modern approaches or feature queries.`
} as const;
...
export const FormatadorMensagens = {
  naoFormatado: (parser: string, detalhes?: string) => {
    const base = `文件 appears to be unformatted (parser: ${parser}). Consider normalizing with Prometheus formatter.`;
...
export const CssInJsMensagens = {
  ...
  globalStyles: (fonte: 'styled-components' | 'styled-jsx') => `Global styles 检测到 (${fonte}). Prefer local scope when possible.`,
...
export const PythonMensagens = {
  ...
  hardcodedString: (string: string) => `Hardcoded string 检测到 (${string.slice(0, 30)}...); consider using constants.`,
